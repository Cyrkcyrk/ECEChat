...
module.exports = (app) => {
	...
	.get('/channel/:id/fetch/:nb?/:messageID?', middleware.channel, (req, res) => {
		if(req.userData.scope.admin || req.channelData.members.includes(req.userData.id)) 
		{
			let messageID = ""
			if(req.params.messageID)
				...
				if(req.channelData.messages.last && req.channelData.messages.last.id)
					...
					res.status(401).json({
						...
						"code" : 404,
						"name" : "missingMessage",
						"message" : "There is no message to fetch"
					})
				}
			}
			...
			if(req.userData.scope.admin)
				adminStatus = true
			db.channels.fetch(req.params.id, messageID, adminStatus, req.params.nb).then(messages => {
				res.json(messages)
			})
			.catch (e => {
				// console.error(e)
